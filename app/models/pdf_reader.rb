class Pdf_reader
	attr_accessor :file, :page, :full_content, :info_content, :mod_content, :y_precision, :col_position, :max; :is_col_set

	def initialize(file,page,columns)
		options = {:y_precision =>5}
	# def initialize(file,page)
		# @file = PDF::Reader.new(file)
		@file = PDF::Reader::Turtletext.new(file, options)
		@page = page
		# @is_field = ["Total Revenues", "Operating Expenses", "Operating Income", "Total Other Income (Expense)", "Provision for Income Tax", "Net Income"]		@years = years
